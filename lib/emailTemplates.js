  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
    <tr><td align="center">
      <table cellpadding="0" cellspacing="0">
        <tr><td align="center" valign="middle" style="background:#fff5f5;border:2px solid #fecaca;border-radius:50%;width:72px;height:72px;font-size:34px;text-align:center;">${emoji}</td></tr>
      </table>
    </td></tr>
  </table>`
}
...
export function financingAdmin({ firstName, lastName, phone, email, employmentStatus, monthlyIncome, loanAmount, downPayment, vehicleYear, vehicleMake, vehicleModel }) {
  return baseLayout({
    body: `
      ...
      ${detailsTable([
        ...
        ['Monthly Income',   monthlyIncome ? `$${monthlyIncome}` : null],
        ['Loan Amount',      loanAmount    ? `$${loanAmount}`    : null],
        ['Down Payment',     downPayment   ? `$${downPayment}`   : null],
        ['Vehicle Interest', [vehicleYear, vehicleMake, vehicleModel].filter(Boolean).join(' ') || null],
      ])}
