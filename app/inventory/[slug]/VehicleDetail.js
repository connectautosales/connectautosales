        ...
        <div className="container">
          <div className={styles.layout}>
            ...
            <div className={styles.left}>
              ...
              {(() => {
                let features = []
                try { features = JSON.parse(car.features || '[]') } catch {}
                if (!Array.isArray(features)) features = []
                return features.length > 0 ? (
                  <div className={styles.descSection}>
                    <h2 className={styles.sectionTitle}>Features & Highlights</h2>
                    <div className={styles.sectionLine} />
                    <ul className={styles.featuresList}>
                      {features.map((f, i) => (
                        <li key={i} className={styles.featureItem}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e50202" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                          {f}
                        </li>
                      ))}
                    </ul>
